<template>
  <div>
    <el-input placeholder="请输入内容" v-model="searchText" id="novel" @keyup.enter.native="search()">
    ...
    <el-card class="box-card">
      ...
      <div v-for="(item,index) in searchData2" :key="index" class="text item">
        {{index+1}} : <span @click="searchGenre(searchData1[index][0])">{{item[0]}}</span>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //去查看小说
      searchNovel(url) {
        this.$store.dispatch('setNovelUrl', url);
